...
TestCase('BMS-176. マスタ:ラボ管理作成:就業先コード:文字種', async () => {
    ...
    await addWorkingPlacePage.enterTextFieldByLabel(WORKING_PLACE_CODE_FIELD_NAME, Constants.hiraganaKatakanaString);
    await addWorkingPlacePage.clickButtonByIcon(ButtonIcon.SAVE);
    gondola.report(`VP. 「半角英数で入力してください」という文字種誤りのエラーが表示されること。`);
    //BUG: invalid feedback is not correct
    await gondola.checkEqual(
    ...
    await addWorkingPlacePage.enterTextFieldByLabel(WORKING_PLACE_CODE_FIELD_NAME, Constants.symbolString);
    await addWorkingPlacePage.clickButtonByIcon(ButtonIcon.SAVE);
    gondola.report(`VP. 「半角英数で入力してください」という文字種誤りのエラーが表示されること。`);
    //BUG: invalid feedback is not correct
    await gondola.checkEqual(
    ...
    await addWorkingPlacePage.enterTextFieldByLabel(WORKING_PLACE_CODE_FIELD_NAME, Utilities.getRandomText(15));
    await addWorkingPlacePage.clickButtonByIcon(ButtonIcon.SAVE);
...
// TestCase('BMS-177. 案件:得意先マスタ作成:顧客情報:取引先コード :重複時', async () => {
...
//     await addWorkingPlacePage.enterTextFieldByLabel(WORKING_PLACE_CODE_FIELD_NAME, randomExistedCustomer.cd);
//     await addWorkingPlacePage.clickButtonByIcon(ButtonIcon.SAVE);
//     gondola.report(
