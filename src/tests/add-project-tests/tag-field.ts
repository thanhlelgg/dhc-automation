...
TestCase('BMS-48. 案件:案件作成:タグ:タグ入力', async () => {
    ...
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_1), 'Tag should be displayed');
    ...
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_2), 'Tag should be displayed');
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_3), 'Tag should be displayed');
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_4), 'Tag should be displayed');
    ...
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_5_UPPERCASE), 'Tag should be displayed');
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_5_LOWERCASE), 'Tag should be displayed');

    gondola.report(`Step 4.2. もう一回入力したタグと同じ大文字のタグを入力する。`);
    await addProjectPage.enterTextFieldByLabel(TAG_FIELD_NAME, TAG_5_UPPERCASE + '\n');

    gondola.report(`VP. 直前まで入力していた文字列がタグとしてまとめられること。`);
    await gondola.checkTrue(await addProjectPage.doesTagUnique(TAG_5_UPPERCASE), 'Tag should be unique');
    ...
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_FULL_WIDTH), 'Tag should be displayed');
    await gondola.checkTrue(await addProjectPage.doesTagDisplay(TAG_HALF_WIDTH), 'Tag should be displayed');

    gondola.report(`Step 5.2. もう一回入力したタグと同じ全角のタグを入力する。`);
    await addProjectPage.enterTextFieldByLabel(TAG_FIELD_NAME, TAG_FULL_WIDTH + '\n');

    gondola.report(`VP. 重複の二つ目の全角のタグはEnterした後、自動的に消えること。`);
    await gondola.checkTrue(await addProjectPage.doesTagUnique(TAG_FULL_WIDTH), 'Tag should be unique');
});
...
TestCase('BMS-49. 案件:案件作成:タグ:タグ削除', async () => {
    ...
    await gondola.checkFalse(await addProjectPage.doesTagDisplay(TAG_1, false), 'Tag should not be displayed');
