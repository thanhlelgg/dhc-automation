...
export default async function setup(): Promise<void> {
    ...
    gondola.report(
        `Step 1. 水平メニューで「営業管理」をクリックして、垂直メニューで「マスタ」→「テンプレート」の「見積書」をクリックします。`,
    );
    await loginPage.gotoBusinessSystem();
    await businessSystemPage.gotoEstimateTemplatePage();
    gondola.report(`VP. 見積書テンプレートファイルの新規登録の画面に移動すること。`);
    gondola.checkTrue(await estimateTemplatePage.isCurrentPage());
}
