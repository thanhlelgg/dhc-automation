...
const CUSTOMER_IS_DISABLE = Object.values(Constants.CUSTOMER_IS_DISABLE);
...
Data(CUSTOMER_IS_DISABLE).TestCase('BMS-458. マスタ:得意先検索:表示対象', async (current: any) => {
    ...
    await gondola.checkTrue(
        await listCustomerPage.doesSelectorByLabelOptionsExist(
            CUSTOMER_IS_DISABLE_FIELD_NAME,
            CUSTOMER_IS_DISABLE.concat(''),
        ),
    ...
    await listCustomerPage.searchCustomer({ isDisable: current });
